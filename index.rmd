...
# read in the raw TESTING data, to be subdivided into two datasets for developing the model.
rawData <- read.csv("pml-training.csv",header=TRUE, sep=",")
rawTesting <- read.csv("pml-testing.csv",header=TRUE,sep=",")
# rawData  <- read.csv("https://d396qusza40orc.cloudfront.net/predmachlearn/pml-training.csv",header=TRUE,sep=",")
...
A model based on the simplified dataset can be performed using tree partitioning. This is a relatively quick process, compared to using random forests, for example.  A plot of the decision tree is shown.
...
# training using tree based partiion, to create a prediction model
modFitRP <- train(classe ~ .,data=training,method="rpart")

# plotting decision tree based on the model
fancyRpartPlot(modFitRP$finalModel)

# create a matrix of predictions for the testing data based on the decision tree.
predRP <- predict(modFitRP,newdata=testing)
...
The process I've outlined is a first-pass at prediction, a starting point for further exploration.  The predictions based on this partition tree model are not particularly accurate, and the use of random forest prediction might be preferable.  However, given time constraint for the class it is simply the best I can do with limited time.
